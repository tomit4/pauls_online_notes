...
Let's first set up the equation that we'llve need to solve.
...
**(b)** we copound interest 6 times a year.
...
Do NOT make the mistake of assuming that it will be approximatelyn 1 day for
this answer based on the answer to the previous part. With exponential growth
things just don't work that way as we'll see. In order to answer this part we
will need to solve the following exponential equation.
...
hours if we just double the time rom the first part. In other words, be careful!
...
The **Richter scale** si commonly used to measure the intensity of an
...
There really isn't mukch to do here other than to plug into the formula.
...
**6.** For a particular radioactive element the value of k in the exonential
