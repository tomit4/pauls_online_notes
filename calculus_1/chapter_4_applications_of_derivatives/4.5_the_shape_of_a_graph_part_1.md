...
A relative minimum is a critical point, so we therefore know it must evaluate to
$0$ for the derivative of $f(x)$:

$$ f'(x) = x + 4 $$

For this critical point to be a relative minimum, it must be decreasing on the
left side of $-4$ and increasing on the right side of $-4$:

$$ f'(-5) = -5 + 4 = -1 \text{ decreasing} $$

$$ f'(-4) = -4 + 4 = 0 $$

$$ f'(-3) = -3 + 4 = 1 \text{ increasing} $$
...
$$ f(-4) = \frac{1}{2}(-4)^2 + 4(-4) + c = 8 - 16 + c = 6 $$
...
Let's go a bit faster through this one.

**(a)**

$$ f'(x) = x + 1 $$

Relative minimum means it is decreasing on left, increasing on right.

$$ f'(-2) = -2 + 1 = -1 \text{ decreasing} $$

$$ f'(-1) = -1 + 1 = 0 $$

$$ f'(0) = 0 + 1 = 1 \text{ increasing} $$
...
Now we just need the general form of the function:

$$ \boxed{f(x) = \frac{1}{2}x^2 + x} $$

And yeah, you can add any constant:
...
$$ \boxed{f(x) = \frac{1}{2}x^2 + x + \frac{7}{2}} $$

**32.** For some function, $f(x)$, it is known that there is a critical point at
$x = 3$ that is neither a relative minimum or a relative maximum. Answer each of
...
Since $x = 3$ is a critical point, but isn't a relative min or max, we just have
to set it up as before, but there's some slight differences on the left and
right.

$$ f'(x) = (x - 3)^2 $$

For there to be no min or max on the left or right, it has to either be
decreasing on both sides, or increasing on both sides of $x = 3$.

$$ f'(2) = (2 - 3)^2 = 1 \text{ increasing} $$

$$ f'(3) = (3 - 3)^2 = 0 $$

$$ f'(4) = (4 - 3)^2 = 1 \text{ increasing} $$
...
We're just looking for the most general form of the function itself.

$$ \boxed{f(x) = \frac{1}{3}(x - 3)^3} $$

And yeah, add any constant:

$$ f(x) = \frac{1}{3}(x - 3)^3 + c $$

**\(c\)**

$$ f(3) = \frac{1}{3}(3 - 3)^3 + c = 2 $$

$$ 0 + c = 2 $$

$$ \boxed{f(x) = \frac{1}{3}(x - 3)^3 + 2} $$
...
We got two critical points here, one is a relative max and one is a min. We can
set this up easy as a product of the two, but let's establish what each means
individually first.

For $x = 1$, we can establish the basics:

$$ f'(x) = 1 - x $$

Now it's a relative maximum, this means to the left it's increasing, and to the
right it's decreasing:

$$ f'(x) = 1 - x $$

$$ f'(0) = 1 - 0 = 1 \text{ increasing} $$

$$ f'(1) = 1 - 1 = 0 $$

$$ f'(2) = 1 - 2 = -1 \text{ decreasing} $$

Cool, this works. next is $x = 4$:

$$ f'(x) = x - 4 $$

This is a relative minimum, and it means that to the left it must be decreasing,
and to the right it must be increasing.

$$ f'(3) = 3 - 4 = -1 \text{ decreasing} $$

$$ f'(4) = 4 - 4 = 0 $$

$$ f'(5) = 5 - 4 = 1 \text{ increasing} $$

So that works, we should be able to just multiply these two as factors to get
our derivative:
...
The problem with this is that if we now plug in our values, we see that our
first factor gives opposite signs that we desire (_i.e._ it becomes a relative
minimum), by flipping the sign, we actually get what we want:
...
It's simple to find the general function form now:

$$ \boxed{f(x) = \frac{1}{3}x^3 - \frac{5}{2}x^2 + 4x + c} $$

Note however, that for part \(c\), we'll need a common multiple around our
general form (except for the constant), so recall that we can factor out any
constant from our polynomial as well:

$$ \boxed{f(x) = k\left(\frac{1}{3}x^3 - \frac{5}{2}x^2 + 4x\right) + c} $$

**\(c\)**

Let's establish our two formulas:

$$ f(x) = k\left(\frac{1}{3}x^3 - \frac{5}{2}x^2 + 4x\right) + c $$

$$ f(1) = k\left(\frac{1}{3}(1)^3 - \frac{5}{2}(1)^2 + 4(1)\right) + c = 6 $$

$$ f(4) = k\left(\frac{1}{3}(4)^3 - \frac{5}{2}(4)^2 + 4(4)\right) + c = -2 $$

We can solve for one at a time by subtracting one from the other, Let's simplify
each first:

$$ f(1) = k\left(\frac{11}{6}\right) + c = 6 $$

$$ f(4) = k\left(-\frac{8}{3}\right) + c = -2 $$

Now let's subtract one from the other (order doesn't really matter) in order to
get rid of $c$ and solve for $k$:

$$ \left(\frac{11}{6}k + c = 6\right) - \left(-\frac{8}{3}k + c = -2\right) $$

$$ \frac{9}{2}k = 8 $$

$$ k = \frac{16}{9} $$

Now solve the first equation for $c$:

$$ f(1) = \frac{11}{6}\left(\frac{16}{9}\right) + c = 6 $$
...
$$ \boxed{f(x) = \frac{16}{9}\left(\frac{1}{3}x^3 - \frac{5}{2}x^2 + 4x\right) + \frac{74}{27}} $$

**34.** Given that $f(x)$ and $g(x)$ are increasing functions will
...
$$ f'(x) > 0 \text{ increasing} $$

For all values of $x$. We know the same is true of $g(x)$:

$$ g'(x) > 0 \text{ increasing} $$

For all values of $x$.

We cannot however, say that:

$$ h'(x) = f'(x) - g'(x) > 0 $$

The reason for this is dependent on the values of both $f'$ and $g'$ at $x$.
Meaning that if:

$$ g'(x) > f'(x) $$

Then $h'(x) < 0$, and will be decreasing.

Let's set up $f'(x)$ and $g'(x)$ where $h'(x)$ will be decreasing, and also find
there respective functions, $f(x)$ and $g(x)$.

$$ g(x) = 2x $$

$$ f(x) = x $$

The derivatives for these are easy:

$$ g'(x) = 2 $$

$$ f'(x) = 1 $$

Now evaluate for $h'(x)$:

$$ h'(x) = f'(x) - g'(x) $$

$$ h'(x) = 1 - 2 = -1 \text{ decreasing} $$

So that's solves our first question. Now if we wanted $h(x)$ to be an increasing
function, we could simply reverse the roles of $g(x)$ and $f(x)$:
...
Some of the conditions that can make:

$$ h(x) = f(x) - g(x) $$

An increasing function are:
...
To review, the answer from 15 was that the derivative of $h(x)$ is:

$$ h'(x) = 2f(x)f'(x) $$

And our answer to this was that the condition that needed to be met for $h(x)$
to be an increasing function was for:

$$ f(x) > 0 $$

We could also established that another way to meet this condition was for $f(x)$
to always return an exponential in which some positive constant was raised to
the variable $x$, which fulfills the same condition:
...
We could also have an exponential with some negative constant as its base, but
then $x$ would have to be even:

$$ f(x) = n^x \text{ if } x \equiv 0 \pmod{2} $$

Regardless of these other variations on the answer, the essential condition that
must be met in order for $h(x)$ to be increasing is:

$$ f'(x)f(x) > 0 $$

**37.** For a function $f(x)$ determine a single condition on $f(x)$ for which
$h(x) = \left[f(x)\right]^3$ will be an increasing function.

**Solution**

Let's find the derivative to see how this looks:

$$ h'(x) = 3\left[f(x)\right]^2f'(x) $$

If we want to ensure that $h(x)$ will be an increasing function, then we only
need to ensure that $f'(x)$ is positive, as the other factors in this derivative
are guaranteed to be positive. Therefore the only condition that must be met for
$h(x)$ to be an increasing function is:
...
By positive functions, Paul means:

$$ f(x) > 0 \quad \forall x $$

Note that this means that $f'(x)$ could still be negative, just that $f(x)$ will
never produce a negative output. And yes, same for $g(x)$ and $g'(x)$.

Let's valuate for $h'(x)$ to see what's going on here:

$$ h'(x) = f'(x)g(x) + f(x)g'(x) $$

Okay, so we've established that $g(x)$ and $f(x)$ are definitely positive. But
in order for $h'(x)$ to be an increasing function, we need to ensure a few
things. We could just say that both $f'(x)$ and $g'(x)$ need to both be positive
as well, but that's not necessarily true. That's just one condition that would
fulfill our criteria. Let's establish that now:

$$ f'(x) > 0 \text{ and } g'(x) > 0 $$

Okay, but as long as the first term is greater than the second term, the
condition works out as well. This means we can have a negative value for either
$f'(x)$ or $g'(x)$, as long as the first product is always positive and greater
than the second product. So really the condition we want is:

$$ \boxed{f'(x)g(x) + f(x)g'(x) > 0} $$

**39.** Repeat #38 for $h(x) = \dfrac{f(x)}{g(x)}$.

**Solution**

Let's find our derivative so we have any idea of what we're working with:

$$ h'(x) = \frac{f'(x)g(x) - f(x)g'(x)}{\left[g(x)\right]^2} $$

Again, we know that:

$$ f(x) > 0 \quad \forall x \quad \quad g(x) > 0 \quad \forall x $$

And we definitely know the denominator of $h'(x)$ will never be negative nor
zero, so no worries there. We only have to worry about the numerator. The
simplest condition that must be true in order for $h(x)$ to be increasing is:
...
The wording here seems to be meant to throw you off.

Think about it.

$$ f'(x) > 0 \quad \forall x \quad \quad g'(x) > 0 \quad \forall x $$

We should probably grab the derivative for $h(x)$ as well:

$$ h'(x) = f'\left(g(x)\right)g'(x) $$

Since $h(x)$ is directly related to the output of $f(x)$, which is guaranteed to
be an increasing function, and we also know that both $f(x)$ and $g(x)$ are
increasing functions, we know without any doubt that $h(x)$ is an increasing
function.
